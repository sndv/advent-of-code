...
object Day11 {
  ...
  def neighbours(idx: Int, rows: Int, cols: Int): List[Int] = {
    ...
    ).flatMap(x => if (x._2) Some(x._1) else None)
  }
  ...
  def flashes(
    ...
    val fl = octopi.map(o => if (o > 9) 1 else 0)
    val newOctopi = octopi.map(o => if (o > 9) -100 else o)
    if (fl.sum > 0) {
      flashes(
        newOctopi.zipWithIndex.map(o =>
          o._1 + neighbours(o._2, rows, cols).map(fl(_)).sum
        ),
        rows,
        cols,
        fln + fl.sum
  ...
  def simulateCycle(
    ...
    (newOctopi.map(o => if (o < 0) 0 else o), fln)
  }
