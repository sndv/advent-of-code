...
object Day12:
  ...
  def traverse(
    ...
    if curr == "end" then 1
    else if path.filter(s => s == s.toLowerCase).contains(curr) then 0
  ...
  def traverse2(
    ...
    if curr == "end" then 1
    else if path.filter(s => s == s.toLowerCase).contains(curr) then
      if allowRepeat && !List("start", "end").contains(curr) then
