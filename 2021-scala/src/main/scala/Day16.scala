...
object Day16 {
  ...
  def parseLiteralValue(data: String): (String, String) =
    if (data(0) == '0') (data.slice(1, 5), data.splitAt(5)._2)
    else {
  ...
  def parseAll(data: String): (List[Long], Long) = {
    if (data == "" || data.toSet == Set('0')) (List.empty[Long], 0)
    else {
  ...
  def boolValue(b: Boolean): Long = if (b) 1 else 0
